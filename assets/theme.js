...
`;
  ...
  var PredictiveSearchDrawer = class extends DrawerContent {
    ...
    async _doPredictiveSearch(term) {
      const response = await fetch(`${window.themeVariables.routes.predictiveSearchUrl}?q=${encodeURIComponent(term)}&resources[limit]=20&resources[limit_scope]=each&section_id=predictive-search`, {
...
jQuery(document).ready(function(){
  jQuery('.find-main-text-wrap li.mlb').addClass('current');
  jQuery('.find-ur-team-tab-wrap #mlb').addClass('current');
	jQuery('.find-main-text-wrap li').click(function(){
		var tab_id = jQuery(this).attr('data-tab');

		jQuery('.find-main-text-wrap li').removeClass('current');
  ...
  var tid = [];
  $('.find-ur-team-tab-wrap ul').each(function(i){
  ...
  tid.map(ls=>{
    if(tid.length > 1) {
      ...
      if(ls === "mlb"){
        ...
        jQuery('.find-ur-team-tab-wrap #nba').addClass('current');
        jQuery('.find-main-text-wrap li.nba').addClass('current');
      }
      else if(ls === "nfl"){
        jQuery('.find-ur-team-tab-wrap #nfl').addClass('current');
        jQuery('.find-main-text-wrap li.nfl').addClass('current');
      }
    }
...
function Accordion(container) {
    ...
    this.$header.on('click', toggleAccordion.bind(this));
    ...
    function toggleAccordion(evt) {
      ...
      if($el.attr('aria-expanded') == 'true'){
        ...
        $el.parent().siblings().find(selectors.content , this.$container).slideUp(400, function(){
          $(this).attr('aria-hidden', true);
        });
        $el.parent().siblings().find(selectors.header , this.$container).attr('aria-expanded', false);
...
theme.About = (function() {
  ...
  function About(container) {
    ...
    this.updateLocationHash(this.$container);
    this.mobileAccordion(this.$container);
    this.desktopTabs(this.$container);
  }

  About.prototype = $.extend({}, About.prototype, {
    updateLocationHash: function(container) {
      if (window.location.hash !== '') {
        var hash = window.location.hash.replace('#', '');
        var $activeTab = $('[js-tab][data-tab="' + hash + '"]');
        var $activeContent = $('[js-content][data-page="' + hash + '"]');

        $activeTab.addClass('about-sections__list-item--active');
        $('[js-tab]').not($activeTab).removeClass('about-sections__list-item--active');
        $activeContent.addClass('about-sections__content-page--active');
        $('[js-content]').not($activeContent).removeClass('about-sections__content-page--active');

        $(window).on('load', function() {
          $('html, body').animate({ scrollTop: $activeTab.offset().top - 200}, 500);
        });
      } else {
        $(selectors.tab).each(function() {
          if ($(this).hasClass('about-sections__list-item--active') && window.location.hash == '') {
    ...
    mobileAccordion: function(container) {
      $(selectors.content).each(function() {
        if ($(this).hasClass('about-sections__content-page--active')) {
      ...
      $(selectors.accordionHeader).on('click', function() {
        var hash = $(this).siblings().data('page');
        window.location.hash = hash;

        $('[js-tab][data-tab="' + hash + '"]').addClass('about-sections__list-item--active').siblings().removeClass('about-sections__list-item--active');
      });
    },

    desktopTabs: function(container) {
      $(selectors.tab).on('click', function() {
        var tab = $(this).data('tab');

        if (window.location.hash == tab) {
          $(this).addClass('about-sections__list-item--active');
          $(this).siblings().removeClass('about-sections__list-item--active');
        }

        if (!$(this).hasClass('about-sections__list-item--active')) {
          $(this).addClass('about-sections__list-item--active');
          $(this).siblings().removeClass('about-sections__list-item--active');
        ...
        $(selectors.content).each(function() {
          if ($(this).data('page') == tab) {
            $(this).addClass('about-sections__content-page--active');
          } else {
            $(this).removeClass('about-sections__content-page--active');
          }
        });
      });
    }
...
$(document).ready(function() {
  ...
  sections.register('about', theme.About);
});